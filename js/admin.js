(function () {
  ...
  function init() {
    ...
    function getSettings() { const s = JSON.parse(localStorage.getItem('bem_admin_settings') || '{}'); if (!s.password) s.password = 'admin123'; return s; }
    ...
    function setLoggedIn(v) { if (v) localStorage.setItem('bem_admin_logged', 'yes'); else localStorage.removeItem('bem_admin_logged'); }
    function statusLabel(st) { return { pending: 'قيد المراجعة', paid: 'في انتظار التفعيل', active: 'نشط' }[st] || st; }

    function showLogin() { loginView.style.display = 'block'; panelView.style.display = 'none'; }
    ...
    async function loadSubs() {
      try { cachedSubs = await db.adminList(); return cachedSubs; }
    ...
    document.getElementById('adminLoginBtn')?.addEventListener('click', () => {
      ...
      if (pw !== settings.password) { err.style.display = 'block'; err.textContent = 'كلمة المرور غير صحيحة'; document.getElementById('adminPassword').value = ''; return; }
      setLoggedIn(true);
      showPanel();
    });
    ...
    document.getElementById('adminLogoutBtn')?.addEventListener('click', () => { if (!confirm('تسجيل الخروج؟')) return; setLoggedIn(false); showLogin(); });
    ...
    function renderAll() {
      const all = getSubs();
      if (!all.length) { app.innerHTML = '<div class="admin-empty"><p>لا يوجد أي اشتراك مسجل بعد</p><button class="admin-btn" onclick="adminRefresh()" style="margin-top:12px;background:#1565c0;color:#fff;padding:10px 20px;border:none;border-radius:8px;cursor:pointer">إعادة تحميل</button></div>'; return; }
    ...
    function renderSettings() {
      ...
      app.innerHTML = '<div class="admin-settings">'
        ...
        + '</div>'
        + '<div class="admin-actions-card"><h3>إجراءات</h3>'
        + '<button class="admin-btn" style="background:#d32f2f;color:#fff;width:100%;padding:12px;border:none;border-radius:8px;font-weight:600;cursor:pointer" onclick="adminClearAll()">حذف الكل</button>'
    ...
    window.adminExport = () => {
      ...
      aEl.href = url; aEl.download = 'bem-subs-' + new Date().toISOString().slice(0, 10) + '.json'; aEl.click();
    };
    ...
    window.changePassword = () => {
      ...
      const s = getSettings();
      if (!old || !np || !cf) { err.style.display = 'block'; err.textContent = 'املأ الحقول'; return; }
      if (old !== s.password) { err.style.display = 'block'; err.textContent = 'كلمة المرور الحالية خطأ'; return; }
      if (np.length < 6) { err.style.display = 'block'; err.textContent = '6 أحرف على الأقل'; return; }
      if (np !== cf) { err.style.display = 'block'; err.textContent = 'غير متطابقة'; return; }
      s.password = np; saveSettings(s);
      alert('تم التغيير');
